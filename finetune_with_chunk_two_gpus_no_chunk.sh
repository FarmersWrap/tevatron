rm -rf /root/autodl-tmp/tevatron/retriever-two-gpu-no-chunk
deepspeed --include localhost:0,1 --master_port 60002 --module tevatron.retriever.driver.train \
  --deepspeed deepspeed/ds_zero2_safe.json \
  --output_dir /root/autodl-tmp/tevatron/retriever-two-gpu-no-chunk \
...
CUDA_VISIBLE_DEVICES=1 python -m tevatron.retriever.driver.encode  \
  --output_dir=temp \
  --model_name_or_path Qwen/Qwen3-Embedding-0.6B \
  --lora_name_or_path /root/autodl-tmp/tevatron/retriever-two-gpu-no-chunk \
  --bf16 \
...
# recall_100              all     0.9333
# ndcg_cut_10             all     0.6610


# recall_100              all     0.9378
# ndcg_cut_10             all     0.6361

# recall_100              all     0.9367
# ndcg_cut_10             all     0.6469