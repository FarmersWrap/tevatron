rm -rf retriever-two-gpu-chunk-128
deepspeed --include localhost:0,1 --master_port 60000 --module tevatron.retriever.driver.train \
  ...
  --output_dir retriever-two-gpu-chunk-128 \
...
# torchrun --nproc_per_node=2 --master_port 60000 -m tevatron.retriever.driver.train \
...
output_dir=/root/autodl-tmp/tevatron/retriever-two-gpu-chunk-128
CUDA_VISIBLE_DEVICES=1 python -m tevatron.retriever.driver.encode  \
  --output_dir=temp \
  --model_name_or_path Qwen/Qwen3-Embedding-0.6B \
  --lora_name_or_path /root/autodl-tmp/tevatron/retriever-two-gpu-chunk-128 \
  --bf16 \
